...
var LiveCard = {
  ...
  init: function() {
    ...
    this.supportsWebRtc = (typeof navigator.mediaDevices.getUserMedia !== 'undefined' && typeof window.MediaRecorder !== 'undefined');
  ...
  createUI: function() {
            ...
            <div class="livecard-modal-body">
              ...
              <div id="create_video_instructions">
                <div class="livecard-instructions">
                  ...
                  <button type="button" class="btn livecard-btn-modal-submit" id="create_video_card_btn">CREATE VIDEO<span class="d-none d-sm-inline"> CARD</span></button>
                </div>
              </div>
    ...
    var modalCaptureNotSupported = `<div class="livecard-modal fade" id="capture_not_supported_modal" tabindex="-1" role="dialog" aria-labelledby="capture_not_supported_modal_label" aria-hidden="true">
      <div class="livecard-modal-dialog livecard-modal-dialog-centered" role="document">
        <div class="livecard-modal-content">
    ...
    var modalChooseImage = `<div class="livecard-modal fade" id="choose_image_modal" tabindex="-1" role="dialog" aria-labelledby="choose_image_modal_label" aria-hidden="true">
      <div class="livecard-modal-dialog livecard-modal-dialog-centered" role="document">
        <div class="livecard-modal-content">
    ...
    var modalTextGiftMsg = `<div class="livecard-modal fade" id="gift_msg_modal" tabindex="-1" role="dialog" aria-labelledby="gift_msg_modal_label" aria-hidden="true">
      <div class="livecard-modal-dialog livecard-modal-dialog-centered" role="document">
        <div class="livecard-modal-content">
          <div class="livecard-modal-body">
    ...
    var modalCardCreated = `<div class="livecard-modal fade" id="card_created_modal" tabindex="-1" role="dialog" aria-labelledby="card_created_modal_label" aria-hidden="true">
      <div class="livecard-modal-dialog livecard-modal-dialog-centered" role="document">
        <div class="livecard-modal-content">
          <div class="livecard-modal-body">
  ...
  resetVideoContainer: function() {
    ...
    document
      ...
      .classList.remove("livecard-fade-show", "livecard-fade-show-start");
    document.getElementById("video-container").style.display = "";
  },

  addEventHandlers: function() {
    let modalCloseBtns = document.querySelectorAll(".livecard-modal-close");
    [...modalCloseBtns].forEach(modal => {
      modal.addEventListener("click", event => {
        this.hideModal("#" + event.target.closest(".livecard-modal").id);
    ...
    document
      ...
      .addEventListener("click", () => this.btnRecordClick());

    document
      .querySelector("#btnStop")
      .addEventListener("click", () => this.btnStopClick());

    document
      .querySelector("#btnRetake")
      .addEventListener("click", () => this.btnRetakeClick());

    document
      .querySelector("#btnPlay")
      .addEventListener("click", () => this.btnPlayClick());

    document
      .querySelector("#btnUse")
      .addEventListener("click", () => this.btnUseClick());
    ...
    document
      ...
      .addEventListener("click", () => {
        if (!this.isMobile) {
          document
            .querySelector("#create_video_instructions")
            .classList.add("livecard-fade-out");
          ...
          setTimeout(function() {
            ...
            document
              .querySelector("#create_video_instructions")
              .classList.remove("livecard-fade-out");
    ...
    document
      .querySelector("#btnChooseVideo")
      .addEventListener("click", () => this.selectVideoFile());

    document
      .querySelector("#btnImageFromWebcam")
      .addEventListener("click", () => {
    ...
    document
      .querySelector("#btnImageFromDisk")
      .addEventListener("click", () => this.selectImageFile());

    document
      .querySelector("#create_text_card_btn")
      .addEventListener("click", () => {
        document
          .querySelector("#create_text_instructions")
          .classList.add("livecard-fade-out");
    ...
    document
      .querySelector("#submit_text_card_btn")
      .addEventListener("click", () => {
        this.hideModal("#gift_msg_modal");
        this.giftMessage = document.querySelector("#textGiftMessage").value;
        this.giftTextInputCallback();
      });

    var phoneMask = new IMask(document.querySelector(".phone_us"), {
      mask: "(000) 000-0000",
    ...
    document.querySelector("#btnFinish").addEventListener("click", () => {
      this.hideModal("#card_created_modal");
      this.recipientPhone = document.querySelector("#txtPhone").value;
      this.phoneInputCallback();
    });

    document
      .querySelector(".livecard-modal-back-arrow")
      .addEventListener("click", () => {
        this.hideModal("#card_created_modal");
  ...
  btnRetakeClick: function() {
    if (this.messageType === "image") {
      ...
      document.querySelector("#btnRetake").style.display = "none";
      document.querySelector("#btnUse").style.display = "none";

      document.querySelector("#btnRecord").style.display = "inline";
    } else {
      this.recordedBlobs = [];

      if (this.supportsWebRtc) {
        document.querySelector("#recorded").style.display = "none";
        document.querySelector("#capture").style.display = "block";
      } else {
        document.getElementById("LCCapture").record();
      }
  ...
  btnUseClick: function() {
    if (this.messageType === "image") {
      ...
      //TODO: for now we just assume success from Flash app
      if (this.recordedBlobs.length > 0 || !this.supportsWebRtc) {
        this.videoRecordSuccessCallback();
      } else {
        this.videoRecordFailureCallback(LiveCardError.RECORDING_FAILED);
      }
    }
  },

  showStaticVideo: function() {
    ...
    return; //workaround below seems to cause autoplay & loop so no thanks
  ...
  generateFlash: function() {
    // For version detection, set to min. required Flash Player version, or 0 (or 0.0.0), for no version detection. 
      ...
      this.flashStreamName = this.uuidv4();
      ...
      swfobject.embedSWF(
          "livecard-sdk/flash/LCCapture.swf?cb=" + (new Date()).getTime(), "flashContent", 
          "100%", "100%", 
  ...
  getImageFromWebcam: function(params) {
    this.imageChooseSuccessCallback = params.onSuccess;
    this.imageChooseFailureCallback = params.onFailure;

    this.messageType = "image";

    document.querySelector("#btnImageFromWebcam").click();
  },
  ...
  hideModal: function(id) {
    ...
    if (id === "#video_gift_msg_modal") {
      document.querySelector(id).classList.remove("showing-video-container");
      document.querySelector("#create_video_instructions").style.display =
        "block";
      document.querySelector("#video-container").style.display = "none";
    }

    if (id === "#gift_msg_modal") {
      document.querySelector("#create_text_instructions").style.display =
        "block";
      document.querySelector("#text-container").style.display = "none";
    }

    this.checkIfModalsOpen();
  },

  showModal: function(id) {
    document.querySelector(id).classList.add("show");

    if (id === "#card_created_modal") {
      let modalTitle =
        this.messageType === "text"
      ...
      document
        .querySelector(id)
        .querySelector(".livecard-modal-title").innerHTML = modalTitle;
    }
  ...
  hideAllModals: function() {
    let modals = document.querySelectorAll(".livecard-modal");

    [...modals].forEach(modal => this.hideModal("#" + modal.id));
  },

  checkIfModalsOpen: function() {
    let modalOpen = false;
    document.querySelectorAll(".livecard-modal").forEach(modal => {
      if (modal.classList.contains("show")) modalOpen = true;
    });
    document.body.classList.toggle("livecard-modal-open", modalOpen);
  },
  ...
  videoCaptureNotSupported: function(error) {
    this.debugLog("navigator.getUserMedia error: ", error);
  ...
  createCard: function(params) {
    ...
    if (
      this.messageType === "video" &&
      ((!this.usingFileInput && this.recordedBlobs.length === 0 && this.supportsWebRtc) ||
        (this.usingFileInput &&
    ...
    request.addEventListener("load", () => {
      ...
      if (this.messageType === "video") {
        if (this.supportsWebRtc) {
          ...
          this.uploadFlashVideo(request.response.card.id);
        }
      } else {
