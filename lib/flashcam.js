class FlashVideoRecorder {
  ...
  init(viewId) {
    ...
    const params = { quality: 'high', bgcolor: '#000000', allowscriptaccess: 'always', allowfullscreen: 'true' };
    const attributes = { id: this.recorderId, name: this.recorderId };

    // TODO: figure out why video does not stream when running on localhost.
    // swfobject.embedSWF(`/livecard-sdk/flash/LCCapture.swf?cb=${(new Date()).getTime()}`, viewId, '100%', '100%',
    //   swfVersionStr, xiSwfUrlStr, flashvars, params, attributes);
    swfobject.embedSWF(`https://retailer.live.cards/checkout/livecard-sdk/flash/LCCapture.swf?cb=${(new Date()).getTime()}`, viewId, '100%', '100%',
      swfVersionStr, xiSwfUrlStr, flashvars, params, attributes);
