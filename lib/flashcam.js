class FlashImageRecorder {
  constructor(recorderId) {
    this.recorderId = recorderId;
    this.flashStreamName = uuidv4();
  }

  init(viewId) {
    setCamera(viewId, this.recorderId, this.flashStreamName);
  }

  remove() {
    removeCamera(this.recorderId);
  }
}

class FlashVideoRecorder {
  constructor(recorderId) {
    this.flashStreamName = uuidv4();
    this.recorderId = recorderId;
    this.recordStarted = false;
    this.recordEnded = false;
  }

  init(viewId) {
    setCamera(viewId, this.recorderId, this.flashStreamName);
  }

  streamName() {
    return this.hasData() ? this.flashStreamName : null;
  }

  recordStart() {
    document.getElementById(this.recorderId).startRecording();
    this.recordStarted = true;
  }

  recordStop() {
    document.getElementById(this.recorderId).stopRecording();
    this.recordEnded = true;
  }

  hasData() {
    return this.recordStarted && this.recordEnded;
  }

  reset() {
    this.recordStarted = false;
    this.recordEnded = false;
    document.getElementById(this.recorderId).record();
  }

  remove() {
    removeCamera(this.recorderId);
  }
}

const uuidv4 = () => {
  return 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, c => {
    let r = Math.random() * 16 | 0, v = c == 'x' ? r : (r & 0x3 | 0x8);
    return v.toString(16);
  });
};

const setCamera = (boxId, recorderId, streamName) => {
  const path = '/livecard-sdk/flash/';

  // For version detection, set to min. required Flash Player version, or 0 (or 0.0.0), for no version detection. 
  const swfVersionStr = '16.0.0';

  // To use express install, set to playerProductInstall.swf, otherwise the empty string. 
  const xiSwfUrlStr = `${path}expressInstall.swf`;

  const flashvars = {
    lc_debug: true,
    rtmp_host: '18.234.125.64',
    rtmp_app: 'petro',
    rtmp_inst: '_definst_',
    video_width: 320,
    video_height: 240,
    video_quality: 95,
    stream_name: streamName
  };

  const params = { quality: 'high', bgcolor: '#000000', allowscriptaccess: 'sameDomain', allowfullscreen: 'true' };
  const attributes = { id: recorderId, name: recorderId };
  const swfUrlStr = `${path}LCCapture.swf?cb=${(new Date()).getTime()}`;

  swfobject.embedSWF(swfUrlStr, boxId, '100%', '100%', swfVersionStr, xiSwfUrlStr, flashvars, params, attributes);
  swfobject.createCSS(`#${boxId}`, 'display:block;text-align:left;');
};

const removeCamera = (recorderId) => {
  swfobject.removeSWF(recorderId);
};

export { FlashImageRecorder, FlashVideoRecorder };